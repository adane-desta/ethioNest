...
const Navbar = () => {
  ...
  const toggleReplyForm = (id, e) => {
    e.stopPropagation();
    e.preventDefault();
    setReplyingMessageId(replyingMessageId === id ? null : id);
    setReplyText('');
    if (expandedMessageId !== id) setExpandedMessageId(id);
  };
  ...
  const groupByDate = (items, dateKey) => {
    ...
    return items.reduce((groups, item) => {
      ...
      let group = itemDate >= today ? 'Today' : 
                 itemDate >= yesterday ? 'Yesterday' : 'Earlier';
    ...
    <>
      <nav>
        <div className="logo">
          ...
          <span className="logo-text">Sidama<span>Rent</span></span>
        </div>
        ...
        <div className="message-container active" ref={messagesRef}>
          ...
          <div className="message-content">
            {isLoadingMessages ? (
              ...
              Object.entries(groupByDate(messages, 'sent_at')).map(([group, groupMessages]) => (
                <div className="message-group" key={group}>
                  <div className="group-title">{group}</div>
                  
                  {groupMessages
                    .filter(msg => activeMessageTab === 'all' || !msg.is_read)
                    .map(message => (
                      <div 
                        key={message.id}
                      ...
                      >
                        <img src={message.sender_avatar} alt="User" className="message-avatar" />
        ...
        <div className="notification-container active" ref={notificationsRef}>
          ...
          <div className="notification-content">
            {isLoadingNotifications ? (
              ...
              Object.entries(groupByDate(notifications, 'created_at')).map(([group, groupNotifications]) => (

import"./modulepreload-polyfill-B5Qt9EMX.js";const c=document.getElementById("requests-nav"),p=document.getElementById("feedbacks-nav");document.getElementById("company-nav");const y=document.getElementById("properties-nav"),f=document.getElementById("users-nav"),m=document.getElementById("analytics-nav"),o=document.getElementById("contentArea");document.getElementById("modal");let n="";const a="https://ethinest.onrender.com/api";m.addEventListener("click",async()=>{u()});const u=async()=>{const r=await(await fetch(`${a}/platform_analytics`)).json();o.innerHTML=`
    ...
    `};f.addEventListener("click",()=>{n="users",v()});const v=async()=>{try{const t=await fetch(`${a}/getusers`);if(!t.ok)throw new Error("Failed to fetch users");const r=await t.json();o.innerHTML=`
                    ...
                    <tbody>
                        ${i.map(s=>`
                            <tr>
                                ...
                                <td>${s.status}</td>
                                <td>
                                    <button class="action-btn accept property" data-propertyId="${s.id}">See More</button>
                                    <button class="action-btn reject property" data-propertyId="${s.id}">Delete</button>
                                </td>
                            </tr>`).join("")}
                    </tbody>
                </table>
            </div>
        `,document.getElementById("user-selector").addEventListener("change",s=>{const l=s.target.value;d(l,l)})}catch(e){console.error("Error fetching properties:",e),o.innerHTML=`<div class="error">Error loading properties: ${e.message}</div>`}document.querySelectorAll(".action-btn.accept.property").forEach(e=>{e.addEventListener("click",function(){const i=this.getAttribute("data-propertyId");h(i)})}),document.querySelectorAll(".action-btn.reject.property").forEach(e=>{e.addEventListener("click",function(){const i=this.getAttribute("data-propertyId");w(i)})})};async function w(t){if((await Swal.fire({title:"Are you sure?",text:"This property will be permanently deleted!",icon:"warning",showCancelButton:!0,confirmButtonColor:"#3085d6",cancelButtonColor:"#d33",confirmButtonText:"Yes, delete it!"})).isConfirmed)try{if((await fetch(`${a}/deleteProperty?property_Id=${t}`,{method:"DELETE"})).ok)Swal.fire({icon:"success",title:"Deleted!",text:"Property has been deleted.",toast:!0,position:"center",showConfirmButton:!1,timer:3e3}),d("all","All");else throw new Error("Failed to delete property")}catch{Swal.fire({icon:"error",title:"Error",text:"Failed to delete property"})}}p.addEventListener("click",async()=>{try{const t=await fetch(`${a}/getfeedbacks`);if(!t.ok)throw new Error("Failed to fetch feedbacks");const r=await t.json();o.innerHTML=`
        ...
        `,n="feedbacks"}catch(t){console.error("Error fetching feedbacks:",t),o.innerHTML=`<div class="error">Error loading feedbacks: ${t.message}</div>`}document.querySelectorAll(".action-btn.accept.feedback").forEach(t=>{t.addEventListener("click",function(){const r=this.getAttribute("data-propertyId");g(r)})})});c.addEventListener("click",async()=>{try{const t=await fetch(`${a}/getproperties/pending`);if(!t.ok)throw new Error("Failed to fetch property requests");const r=await t.json();o.innerHTML=`
